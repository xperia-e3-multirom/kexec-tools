...
static uint32_t flamingo_dtb_compatible(void *dtb, struct msm_id *devid, struct msm_id *dtb_id)
{
    int root_offset;
    const void *prop;
    int len;
...
static int flamingo_choose_dtb(const char *dtb_img, off_t dtb_len, char **dtb_buf, off_t *dtb_length)
{
    ...
    while(dtb + sizeof(struct fdt_header) < dtb_end)
    {
        ...
        if(flamingo_dtb_compatible(dtb, &devid, &dtb_id))
        {
...
static int flamingo_add_extra_regs(void *dtb_buf)
{
    FILE *f;
    uint32_t reg;
    int res;
...
const struct arm_mach arm_mach_flamingo = {
    .boardnames = { "flamingo", "d2202", "D2202", "d2203", "D2203", "d2206", "D2206", "d2212", "D2212", "d2243", "D2243", NULL },
    .choose_dtb = flamingo_choose_dtb,
    .add_extra_regs = flamingo_add_extra_regs,
};
